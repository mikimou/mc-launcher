...
func checkUpdate(url string) {
	...
	if jsonMap["name"] != string(version) {
		fmt.Println(lipgloss.JoinHorizontal(lipgloss.Top, stat.Render(" VER "), line.Render("DOSTUPNA NOVA VERZIA!"), check.Render(" OK ")))
	}

	fmt.Println(lipgloss.JoinHorizontal(lipgloss.Top, stat.Render(" VER "), line.Render("Aktualna verzia: "+string(version)), check.Render(" OK ")))
}
